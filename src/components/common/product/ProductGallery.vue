...
<template>
  <div class="flex flex-col gap-4">
    <div
      class="relative aspect-square rounded-2xl overflow-hidden bg-muted group cursor-zoom-in"
      style="border: 1px solid oklch(0.88 0.008 85)"
      @click="zoomOpen = true"
    >
      <img
        :src="activeImage?.url"
      ...
      />
      <div
        class="absolute top-3 right-3 p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200"
        style="background: oklch(1 0 0 / 0.85); backdrop-filter: blur(8px)"
      >
    ...
    <Teleport to="body">
      <Transition name="fade">
        ...
        >
          <div
            class="relative max-w-3xl w-full max-h-[90vh] rounded-2xl overflow-hidden"
          >
            <img
              :src="activeImage?.url"
              :alt="activeImage?.alt || productName"
