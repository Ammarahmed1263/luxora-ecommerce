  ...
  <aside class="w-64 shrink-0 space-y-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h2 class="font-bold text-foreground text-sm uppercase tracking-wider">
          Filters
        </h2>
      </div>
      <button
    ...
    <div class="space-y-2">
      <h3
        class="text-xs font-extrabold uppercase tracking-wider text-muted-foreground mb-3"
      >
        Category
      </h3>
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center gap-2">
        <input
          ...
          @change="
            emit('update:priceMin', +($event.target as HTMLInputElement).value)
          "
          type="number"
          placeholder="Min"
        ...
        <input
          ...
          @change="
            emit('update:priceMax', +($event.target as HTMLInputElement).value)
          "
          type="number"
          placeholder="Max"
          class="h-9 w-full rounded-xl border border-border/80 px-3 text-xs focus:outline-none focus:border-primary bg-white text-foreground"
        ...
        >
          <span class="text-amber-400 flex">
            ...
            <Star
              ...
              :key="i"
              :size="12"
              class="stroke-[2.5] text-muted-foreground/30"
            />
          </span>
          <span>& above</span>
        </button>
